...
def test_gpb_py07_ledger_counts_are_frozen():
    counts = parity_counts()
    assert counts == {
        "implemented": 35,
        "implemented_initial": 1,
        "mapped_not_implemented": 422,
    }
...
def test_gpb_py07_reference_fixture_and_runtime_evidence_are_closed():
    ...
    evidence_path = (
        ROOT / "dev/parity/posterior_predictive_backend_validation_0.1.0.dev0.json"
    )
...
def test_gpb_py07_runtime_backend_evidence_is_conservative():
    ...
    assert evidence["validated_commit_sha"] == (
        "fd046c267c5b3cc8bc4f778b58dcf6713ebae5e6"
    )
